...
{
    private $valid_types = ['above', 'under'];

    public function postCreate()
    {
        if (!Auth::check()) {
            return ['success' => false, 'errors' => 'You must be logged in to place any bets.'];
        }
        ...
        if ($bet_amount > Auth::user()->getBalance()) {
            return ['success' => false, 'errors' => ['bet_amount' => 'You don\'t have enough funds to place that bet.']];
        }

        if ($bet_amount < Config::get('bitcoin.minimum_bet')) {
            return ['success' => false, 'errors' => ['bet_amount' => 'Minimum bet amount is '.Bitcoin::toBTC(Config::get('bitcoin.minimum_bet')).' BTC']];
        }

        if ($cross_bet_amount < Config::get('bitcoin.minimum_bet')) {
            return ['success' => false, 'errors' => ['cross_bet_amount' => 'Minimum bet amount is '.Bitcoin::toBTC(Config::get('bitcoin.minimum_bet')).' BTC']];
        }

        $type = Input::get('type');
        if (!in_array($type, $this->valid_types)) {
            return ['success' => false, 'errors' => ['type' => 'Invalid.']];
        }
        ...
        if ($expiration < Config::get('bitcoin.minimum_bet_expiration') ||
            ...
            return ['success' => false, 'errors' => ['expiration' => 'Invalid.']];
        }
        ...
        if ($bet->save()) {
            ...
            return ['success' => false, 'errors' => 'Bet couldn\'t be created.'];
        }
    }
    ...
    public function postCrossbet()
    {
        ...
        if (is_null($bet)) {
            return ['success' => false, 'errors' => 'This is not a valid bet!'];
        }
        if (time() > strtotime($bet->expires_at)) {
            return ['success' => false, 'errors' => 'This bet has expired!'];
        }
        if (!is_null($bet->accepted_by_user_id)) {
            return ['success' => false, 'errors' => 'Somebody else just placed this bet!'];
        }
        if ($bet->deleted != 0) {
            return ['success' => false, 'errors' => 'This bet has been deleted!'];
        }
        if ($bet->cross_bet_amount > Auth::user()->getBalance()) {
            return ['success' => false, 'errors' => 'You don\'t have enough funds to place this bet!'];
        }
        ...
        if ($bet->save()) {
            ...
            return ['success' => false, 'errors' => 'Bet couldn\'t be placed, please contact support if this persists.'];
        }
    }
