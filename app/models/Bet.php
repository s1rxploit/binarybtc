...
class Bet extends Eloquent
{
    ...
    public function scopeAvailable($query)
    {
        ...
        return $query->where('expired', 0)->whereNull('accepted_by_user_id')->where('deleted', 0)->whereNotIn('created_by_user_id', [$user_id]);
    }
    ...
    {
        $expired = self::available()->where('expires_at', '<=', date('Y-m-d H:i:s'))->whereNull('accepted_by_user_id')->lists('id');
        if (count($expired) > 0) {
            self::whereIn('id', $expired)->update(['expired' => 1]);
        }

        $finished = self::where('expires_at', '<=', date('Y-m-d H:i:s'))->whereNotNull('accepted_by_user_id')->where('winner_paid', 0)->get();
    ...
    {
        $this->update(['deleted' => 1, 'price_when_deleted' => $btc_price, 'deleted_at' => time()]);
    }
