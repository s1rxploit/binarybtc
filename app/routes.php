...
App::before(function ($request) {
    if ($request->getMethod() === 'POST') {
        Route::callRouteFilter('custom_csrf', [], '', $request);
    }
});
...
Route::get('/latest_price', function () {
    ...
    return ['price' => number_format($latest->price, 4), 'time' => strtotime($latest->created_at), 'date' => $latest->created_at];
});
...
Route::get('/update_price', ['before' => 'internal_only', function () {
